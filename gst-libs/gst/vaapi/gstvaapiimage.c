...
static gboolean _gst_vaapi_image_map (GstVaapiImage * image);
...
vaapi_image_get_type (void)
{
  ...
  if (G_UNLIKELY (type == 0))
    type = g_boxed_type_register_static ("VAImage",
        vaapi_image_copy, vaapi_image_free);
  return type;
}
...
_gst_vaapi_image_create (GstVaapiImage * image, GstVaapiImageFormat format)
{
  ...
  status = vaCreateImage (GST_VAAPI_DISPLAY_VADISPLAY (display),
      ...
      priv->width, priv->height, &priv->internal_image);
  GST_VAAPI_DISPLAY_UNLOCK (display);
...
static void
gst_vaapi_image_set_property (GObject * object,
...
static void
gst_vaapi_image_get_property (GObject * object,
...
gst_vaapi_image_class_init (GstVaapiImageClass * klass)
{
  ...
  g_object_class_install_property
      ...
      g_param_spec_boxed ("image",
          ...
          VAAPI_TYPE_IMAGE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
...
gst_vaapi_image_new (GstVaapiDisplay * display,
    GstVaapiImageFormat format, guint width, guint height)
{
  GstVaapiImage *image;
  ...
  image = g_object_new (GST_VAAPI_TYPE_IMAGE,
      "display", display,
...
gst_vaapi_image_new_with_image (GstVaapiDisplay * display, VAImage * va_image)
{
  ...
  image = g_object_new (GST_VAAPI_TYPE_IMAGE,
      "display", display,
      "id", GST_VAAPI_ID (va_image->image_id), "image", va_image, NULL);
  if (!image)
    return NULL;
...
_gst_vaapi_image_map (GstVaapiImage * image)
{
  ...
  status = vaMapBuffer (GST_VAAPI_DISPLAY_VADISPLAY (display),
      image->priv->image.buf, &image_data);
  GST_VAAPI_DISPLAY_UNLOCK (display);
