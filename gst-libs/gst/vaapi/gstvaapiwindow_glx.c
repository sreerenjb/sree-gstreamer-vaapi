...
G_DEFINE_TYPE (GstVaapiWindowGLX,
    gst_vaapi_window_glx, GST_VAAPI_TYPE_WINDOW_X11);
...
static gboolean
_gst_vaapi_window_glx_create_context (GstVaapiWindowGLX * window,
...
static gboolean
_gst_vaapi_window_glx_ensure_context (GstVaapiWindowGLX * window,
    GLXContext foreign_context)
{
  GstVaapiWindowGLXPrivate *const priv = window->priv;
...
static gboolean
gst_vaapi_window_glx_ensure_context (GstVaapiWindowGLX * window,
    GLXContext foreign_context)
{
  GstVaapiWindowGLXPrivate *const priv = window->priv;
...
gst_vaapi_window_glx_resize (GstVaapiWindow * window, guint width, guint height)
{
  ...
  if (!GST_VAAPI_WINDOW_CLASS (gst_vaapi_window_glx_parent_class)->resize
      (window, width, height))
    return FALSE;
...
static void
gst_vaapi_window_glx_set_property (GObject * object,
...
static void
gst_vaapi_window_glx_get_property (GObject * object,
...
gst_vaapi_window_glx_class_init (GstVaapiWindowGLXClass * klass)
{
  ...
  g_object_class_install_property
      ...
      g_param_spec_pointer ("glx-context",
          "GLX context", "GLX context", G_PARAM_READWRITE));
}
...
gst_vaapi_window_glx_new_with_xid (GstVaapiDisplay * display, Window xid)
{
  ...
  return g_object_new (GST_VAAPI_TYPE_WINDOW_GLX,
      "display", display, "id", GST_VAAPI_ID (xid), NULL);
}
