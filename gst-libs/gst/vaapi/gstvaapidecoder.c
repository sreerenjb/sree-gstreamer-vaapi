...
static gboolean
push_surface (GstVaapiDecoder * decoder,
    GstVaapiSurface * surface, GstClockTime timestamp)
{
  GstVaapiDecoderPrivate *const priv = decoder->priv;
...
static void
gst_vaapi_decoder_set_property (GObject * object,
...
static void
gst_vaapi_decoder_get_property (GObject * object,
...
gst_vaapi_decoder_class_init (GstVaapiDecoderClass * klass)
{
  ...
  g_object_class_install_property
      ...
      g_param_spec_pointer ("caps",
          "Decoder caps",
          "The decoder caps", G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}
    ...
    GstVaapiEntrypoint entrypoint, guint width, guint height)
{
  ...
  priv->context = gst_vaapi_context_new (priv->display,
      profile, entrypoint, width, height);
  return priv->context != NULL;
}

gboolean
gst_vaapi_decoder_push_buffer_sub (GstVaapiDecoder * decoder,
...
gboolean
gst_vaapi_decoder_push_surface (GstVaapiDecoder * decoder,
    GstVaapiSurface * surface, GstClockTime timestamp)
{
