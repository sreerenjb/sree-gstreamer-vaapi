...
decode_sequence(GstVaapiDecoderMpeg2 *decoder, guchar *buf, guint buf_size)
{
	...
	GST_DEBUG_OBJECT(decoder, "Calculated PAR value might not be correct..");
...
static gboolean
allocate_dummy_picture (GstVaapiDecoderMpeg2 *decoder) 
{
    GstVaapiDecoderMpeg2Private * const priv = decoder->priv;
    GstVaapiPicture * const picture = priv->current_picture;
    ...
    if (!dummy_picture) {
        GST_ERROR("failed to create dummy picture");
	return FALSE;
    }

    if (!gst_vaapi_picture_allocate_surface(dummy_picture)) {
	GST_ERROR("failed to allocate storage for dummy picture");
	return FALSE;
    }
    ...
    gboolean *have_frame)
{
    ...
    if (type == GST_MPEG_VIDEO_PACKET_PICTURE) {
        if (priv->current_picture && priv->ready_to_dec) { /*needs to start the new frame*/
    ...
    *toadd = ofs;
    ...
    status   = decode_packet(decoder, buf_data, buf_size);
   
    if (buf_data[3] == GST_MPEG_VIDEO_PACKET_SEQUENCE_END) 
	*have_frame = TRUE;
...
decode_buffer(GstVaapiDecoderMpeg2 *decoder, GstBuffer *buffer, GstVideoCodecFrame *frame)
{
	...
	if (picture->type == GST_VAAPI_PICTURE_TYPE_I &&
		...
		if(!allocate_dummy_picture(decoder))
		    GST_ERROR("Failed to add allocate dummy picture");
	}
