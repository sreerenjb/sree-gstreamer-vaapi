...
G_DEFINE_TYPE (GstVaapiDisplayWayland,
    gst_vaapi_display_wayland, GST_VAAPI_TYPE_DISPLAY);
...
static void
gst_vaapi_display_wayland_set_property (GObject * object,
...
static void
gst_vaapi_display_wayland_get_property (GObject * object,
...
gst_vaapi_display_wayland_constructed(GObject *object)
{
    ...
    priv->create_display = priv->wayland_display == NULL;
    ...
    if (priv->create_display) {
        ...
        if (info) {
            priv->wayland_display    = info->native_display;
...
)
{
    ...
    if (!cache)
        return FALSE;
    cached_info = gst_vaapi_display_cache_lookup_by_native_display(cache, priv->wayland_display);
    if (cached_info) {
        *info = *cached_info;
        return TRUE;
    }
...
gst_vaapi_display_wayland_class_init (GstVaapiDisplayWaylandClass * klass)
{
  ...
  g_object_class_install_property
      ...
      g_param_spec_pointer ("wayland-display",
          "Wayland display",
          "Wayland display", G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
    /**
...
gst_vaapi_display_wayland_new_with_display (struct wl_display * wayland_display)
{
  ...
  return g_object_new (GST_VAAPI_TYPE_DISPLAY_WAYLAND,
      "wayland-display", wayland_display, NULL);
}
