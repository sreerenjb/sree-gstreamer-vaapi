...
G_DEFINE_TYPE (GstVaapiDecoderFfmpeg,
    gst_vaapi_decoder_ffmpeg, GST_VAAPI_TYPE_DECODER);
...
get_entrypoint (enum PixelFormat pix_fmt)
{
  switch (pix_fmt) {
    case PIX_FMT_VAAPI_VLD:
      return GST_VAAPI_ENTRYPOINT_VLD;
    case PIX_FMT_VAAPI_IDCT:
      return GST_VAAPI_ENTRYPOINT_IDCT;
    case PIX_FMT_VAAPI_MOCO:
      return GST_VAAPI_ENTRYPOINT_MOCO;
    default:
      break;
  }
  return 0;
...
get_context (AVCodecContext * avctx)
{
  ...
  gst_vaapi_decoder_set_framerate (decoder,
      avctx->time_base.den / avctx->ticks_per_frame, avctx->time_base.num);

  gst_vaapi_decoder_set_pixel_aspect_ratio (decoder,
      avctx->sample_aspect_ratio.num, avctx->sample_aspect_ratio.den);

  success = gst_vaapi_decoder_ensure_context (decoder,
      vactx->profile,
      vactx->entrypoint, avctx->coded_width, avctx->coded_height);
  if (!success) {
...
gst_vaapi_decoder_ffmpeg_get_format (AVCodecContext * avctx,
...
decode_frame (GstVaapiDecoderFfmpeg * ffdecoder, guchar * buf, guint buf_size)
{
  ...
  GST_VAAPI_DISPLAY_LOCK (display);
  bytes_read = avcodec_decode_video2 (priv->avctx,
      priv->frame, &got_picture, &pkt);
  GST_VAAPI_DISPLAY_UNLOCK (display);
  if (!got_picture)
  ...
  surface =
      gst_vaapi_context_find_surface_by_id (GST_VAAPI_DECODER_CONTEXT
      (ffdecoder), GPOINTER_TO_UINT (priv->frame->data[3])
      );
  if (!surface)
...
gst_vaapi_decoder_ffmpeg_decode (GstVaapiDecoder * decoder, GstBuffer * buffer)
{
  ...
  if (priv->pctx) {
    do {
      int parsed_size;
      parsed_size = av_parser_parse2 (priv->pctx,
          priv->avctx,
          &outbuf, &outbuf_size,
...
gst_vaapi_decoder_ffmpeg_new (GstVaapiDisplay * display, GstCaps * caps)
{
  ...
  ffdecoder = g_object_new (GST_VAAPI_TYPE_DECODER_FFMPEG,
