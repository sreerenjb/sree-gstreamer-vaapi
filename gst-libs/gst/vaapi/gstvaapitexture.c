...
_gst_vaapi_texture_create_objects (GstVaapiTexture * texture, GLuint texture_id)
{
  ...
  status = vaCreateSurfaceGLX (GST_VAAPI_OBJECT_VADISPLAY (texture),
      priv->target, texture_id, &priv->gl_surface);
  GST_VAAPI_OBJECT_UNLOCK_DISPLAY (texture);
  ...
  priv->gl_context = gl_create_context (GST_VAAPI_OBJECT_XDISPLAY (texture),
      GST_VAAPI_OBJECT_XSCREEN (texture), &old_cs);
  if (!priv->gl_context || !gl_set_current_context (priv->gl_context, NULL))
    goto end;

  priv->pixo = gl_create_pixmap_object (GST_VAAPI_OBJECT_XDISPLAY (texture),
      priv->width, priv->height);
  if (!priv->pixo)
    goto end;

  priv->fbo = gl_create_framebuffer_object (priv->target,
      texture_id, priv->width, priv->height);
  if (priv->fbo)
    success = TRUE;
end:
  gl_set_current_context (&old_cs, NULL);
...
gst_vaapi_texture_create (GstVaapiTexture * texture)
{
  ...
  if (priv->foreign_texture)
    ...
    GST_VAAPI_OBJECT_LOCK_DISPLAY (texture);
    texture_id = gl_create_texture (priv->target,
        priv->format, priv->width, priv->height);
    GST_VAAPI_OBJECT_UNLOCK_DISPLAY (texture);
    if (!texture_id)
      return FALSE;
...
static void
gst_vaapi_texture_set_property (GObject * object,
...
static void
gst_vaapi_texture_get_property (GObject * object,
    ...
    GLenum target, GLenum format, guint width, guint height)
{
  ...
  return g_object_new (GST_VAAPI_TYPE_TEXTURE,
      ...
      "id", GST_VAAPI_ID (0),
      "target", target,
      "format", format, "width", width, "height", height, NULL);
}
...
gst_vaapi_texture_new_with_texture (GstVaapiDisplay * display,
    GLuint texture, GLenum target, GLenum format)
{
    ...
    GstVaapiSurface * surface, guint flags)
{
  ...
  glBegin (GL_QUADS);
  {
    ...
    glVertex2i (0, priv->height);
    glTexCoord2f (1.0f, 1.0f);
    glVertex2i (priv->width, priv->height);
    glTexCoord2f (1.0f, 0.0f);
    glVertex2i (priv->width, 0);
  }
  glEnd ();
