...
GST_BOILERPLATE_FULL (GstVaapiSink,
    gst_vaapisink, GstVideoSink, GST_TYPE_VIDEO_SINK, gst_vaapisink_iface_init);
...
static gboolean
gst_vaapisink_implements_interface_supported (GstImplementsInterface * iface,
    GType type)
{
  return (type == GST_VAAPI_TYPE_VIDEO_SINK || type == GST_TYPE_X_OVERLAY);
}
...
static GstVaapiDisplay *gst_vaapisink_get_display (GstVaapiSink * sink);
...
static void
gst_vaapisink_xoverlay_set_window_handle (GstXOverlay * overlay,
    guintptr window_id)
{
...
gst_vaapisink_iface_init (GType type)
{
  ...
  G_IMPLEMENT_INTERFACE (GST_TYPE_X_OVERLAY, gst_vaapisink_xoverlay_iface_init);
}
...
static gboolean
configure_notify_event_pending (GstVaapiSink * sink,
    Window window, guint width, guint height)
{
  ...
  XCheckIfEvent (gst_vaapi_display_x11_get_display (GST_VAAPI_DISPLAY_X11
          (sink->display)), &xev, configure_notify_event_pending_cb,
      (XPointer) & args);
  return args.match;
}
#endif
...
#else
    sink->display = gst_vaapi_display_wayland_new (sink->display_name);
#endif
    ...
    guint height)
{
  ...
  gst_vaapi_display_get_pixel_aspect_ratio (sink->display,
...
gst_vaapisink_set_caps (GstBaseSink * base_sink, GstCaps * caps)
{
  ...
  if (!gst_vaapisink_ensure_window (sink, sink->window_width,
          sink->window_height))
    return FALSE;
#else
...
static GstFlowReturn
gst_vaapisink_buffer_alloc (GstBaseSink * base_sink,
    guint64 offset, guint size, GstCaps * caps, GstBuffer ** pout_buffer)
{
  GstVaapiSink *const sink = GST_VAAPISINK (base_sink);
...
render_reflection (GstVaapiSink * sink)
{
  ...
  glBegin (GL_QUADS);
  {
    ...
    glTexCoord2f (1.0f, ry);
    glVertex2i (x2, y1 + rh);
    glTexCoord2f (0.0f, ry);
    glVertex2i (x1, y1 + rh);
  }
  glEnd ();
}
    ...
    GstVaapiSurface * surface, guint flags)
{
  ...
  if (!sink->texture) {
    sink->texture = gst_vaapi_texture_new (sink->display,
        GL_TEXTURE_2D, GL_BGRA, sink->video_width, sink->video_height);
    if (!sink->texture)
...
#if !USE_VAAPISINK_WAYLAND
static inline gboolean
gst_vaapisink_show_frame_x11 (GstVaapiSink * sink,
    GstVaapiSurface * surface, guint flags)
{
...
#if USE_VAAPISINK_WAYLAND
static inline gboolean
gst_vaapisink_show_frame_wayland (GstVaapiSink * sink,
    GstVaapiSurface * surface, guint flags)
{

  sink->display_rect.x = 0;
  sink->display_rect.y = 0;
  sink->display_rect.width = sink->video_width;
  sink->display_rect.height = sink->video_height;
...
#endif
#if USE_VAAPISINK_WAYLAND
    success = gst_vaapisink_show_frame_wayland (sink, surface, flags);
#else
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_USE_GLX:
      ...
      break;
    case PROP_USE_WAYLAND:
      sink->use_wayland = g_value_get_boolean (value);
      break;
    case PROP_DISPLAY:
      g_free (sink->display_name);
    ...
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_USE_GLX:
      ...
      break;
    case PROP_USE_WAYLAND:
      g_value_set_boolean (value, sink->use_wayland);
      break;
    case PROP_DISPLAY:
...
gst_vaapisink_class_init (GstVaapiSinkClass * klass)
{
  ...
  g_object_class_install_property
      ...
      g_param_spec_boolean ("use-glx",
          "GLX rendering", "Enables GLX rendering", TRUE, G_PARAM_READWRITE));
  ...
  g_object_class_install_property
      ...
      g_param_spec_boolean ("use-reflection",
          "Reflection effect",
          "Enables OpenGL reflection effect", FALSE, G_PARAM_READWRITE));
#endif
  ...
  g_object_class_install_property
      (object_class,
      PROP_USE_WAYLAND,
      g_param_spec_boolean ("use-wayland",
          "WAYLAND rendering",
          "Enables WAYLAND rendering", TRUE, G_PARAM_READWRITE));
  ...
  g_object_class_install_property
      ...
      g_param_spec_string ("display",
          ...
          "Wayland display name", "", G_PARAM_READWRITE));
#else
  g_object_class_install_property
      (object_class,
      PROP_DISPLAY,
      g_param_spec_string ("display",
          "X11 display name", "X11 display name", "", G_PARAM_READWRITE));
  g_object_class_install_property
      (object_class,
      PROP_FULLSCREEN,
      g_param_spec_boolean ("fullscreen",
          "Fullscreen",
          "Requests window in fullscreen state", FALSE, G_PARAM_READWRITE));
  ...
  g_object_class_install_property
      ...
      g_param_spec_boolean ("synchronous",
          "Synchronous mode",
          "Toggles X display synchronous mode", FALSE, G_PARAM_READWRITE));
#endif
}
...
gst_vaapisink_init (GstVaapiSink * sink, GstVaapiSinkClass * klass)
{
  ...
  sink->use_wayland = USE_VAAPISINK_WAYLAND;
...
plugin_init (GstPlugin * plugin)
{
  ...
  return gst_element_register (plugin,
      GST_PLUGIN_NAME, GST_RANK_PRIMARY, GST_TYPE_VAAPISINK);
}
