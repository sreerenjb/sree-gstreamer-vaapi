...
GST_ELEMENT_DETAILS ("VA-API decoder",
    "Codec/Decoder/Video",
    GST_PLUGIN_DESC,
  ...
  again:
    ...
    if (!proxy) {
      if (status == GST_VAAPI_DECODER_STATUS_ERROR_NO_SURFACE) {
        ...
        g_mutex_lock (decode->decoder_mutex);
        g_cond_timed_wait (decode->decoder_ready,
            decode->decoder_mutex, &timeout);
        g_mutex_unlock (decode->decoder_mutex);
        goto again;
      }
    ...
    g_object_weak_ref (G_OBJECT (proxy),
        (GWeakNotify) gst_vaapidecode_release, decode);

    buffer = NULL;
    ret = gst_pad_alloc_buffer (decode->srcpad,
        0, 0, GST_PAD_CAPS (decode->srcpad), &buffer);
    if (ret != GST_FLOW_OK || !buffer)
      goto error_create_buffer;

    GST_BUFFER_TIMESTAMP (buffer) = GST_VAAPI_SURFACE_PROXY_TIMESTAMP (proxy);
    gst_vaapi_video_buffer_set_surface_proxy (GST_VAAPI_VIDEO_BUFFER (buffer),
        proxy);
...
gst_vaapidecode_create (GstVaapiDecode * decode, GstCaps * caps)
{
  ...
  g_signal_connect (G_OBJECT (decode->decoder),
      "notify::caps", G_CALLBACK (gst_vaapi_decoder_notify_caps), decode);
...
static void
gst_vaapidecode_set_property (GObject * object,
...
static void
gst_vaapidecode_get_property (GObject * object,
...
gst_vaapidecode_class_init (GstVaapiDecodeClass * klass)
{
  ...
  g_object_class_install_property
      ...
      g_param_spec_boolean ("use-ffmpeg",
          ...
          "Uses FFmpeg/VAAPI for decoding", TRUE, G_PARAM_READWRITE));
}
...
plugin_init (GstPlugin * plugin)
{
  ...
  return gst_element_register (plugin,
      GST_PLUGIN_NAME, GST_RANK_PRIMARY, GST_TYPE_VAAPIDECODE);
}
